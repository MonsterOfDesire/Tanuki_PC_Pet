...
class PetRuntimeStateTests(unittest.TestCase):
    def test_behavior_state_defaults_match_pet_widget_expectations(self):
        ...
        self.assertEqual(state.ambient_low_mood_tag, "")
        self.assertEqual(state.ambient_low_mood_streak, 0)
        self.assertEqual(state.visual_band_afterglow, "")
        self.assertEqual(state.visual_band_afterglow_until, 0.0)
        self.assertEqual(state.pending_ambient_animation_event, ())
        self.assertEqual(state.ambient_animation_event_serial, 0)
        self.assertEqual(
            state.ambient_animation_confirmation_scheduled_serial,
            0,
        )
    ...
    def test_motion_state_defaults_cover_physics_and_movement(self):
        ...
        self.assertFalse(state.throw_active)
        self.assertEqual(state.throw_velocity_x, 0.0)
        self.assertEqual(state.throw_remainder_x, 0.0)
    ...
    def test_state_proxy_field_map_covers_runtime_state_groups(self):
        ...
        self.assertIn("visual_band_afterglow_until", PET_STATE_PROXY_FIELDS["behavior_state"])
        self.assertIn("pending_ambient_animation_event", PET_STATE_PROXY_FIELDS["behavior_state"])
        self.assertIn("ambient_animation_event_serial", PET_STATE_PROXY_FIELDS["behavior_state"])
