...
class DashboardPresenter:
    ...
    def build_household_summary(self, household, entries):
        ...
        entries = self.filter_household_summary_entries(entries)
        if not entries:
            return HouseholdSummaryPresentation(
                title="家庭摘要",
    ...
    def build_social_log(self, entries, filter_mode="all", participant_name=""):
        ...
        title = f"社交紀錄 - {self.SOCIAL_LOG_FILTER_LABELS[filter_mode]}"
    ...
    def build_relationship_table(self, household, pet_names=()):
        ...
        for actor_name in actor_names:
            ...
            for target_name in actor_names:
                ...
                lines.append(self.format_relationship_row(target_name, entry))
            lines.append("")
    ...
    def format_relationship_row(self, target_name, entry):
        ...
        return (
            f"  -> {target_name}: "
            f"好感度 {affinity:5.2f} | "
            f"熟悉 {familiarity:5.2f} / "
            f"信任 {trust:5.2f} / "
            f"依附 {attachment:5.2f} / "
            f"緊張 {tension:5.2f} | "
            f"事件 {event_count}"
        )
