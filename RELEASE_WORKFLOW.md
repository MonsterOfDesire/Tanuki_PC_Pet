...
ZIP 與 manifest 必須由同一次 `tools/build_update_package.py` 執行產生，並和 `build_lab_2.ps1` 產出的更新器一起上傳；ZIP 變動後不得沿用舊 manifest。GitHub Release tag、manifest `version` 與程式內 `tanuki_core/app_version.py` 必須一致。三個資產上傳完成後，先以 `TanukiUpdater.exe --self-check` 驗證實際成品；狀態頁只有在 updater、manifest 與精確版本 ZIP 齊全時才提供直接下載。beta 版本應標記為 prerelease，讓 beta 更新器納入 prerelease、穩定版更新器忽略 prerelease。主程式只保留使用者主動按下的手動查詢，不在啟動時連線 GitHub。
